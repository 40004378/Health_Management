...
const MapComponent = () => {
    ...
    const [selectedFruit, setSelectedFruit] = useState('Casting and machining');
    ...
    function fetchProducts() {
        axios.get('http://localhost:8000/mapData')
            .then(response => {
    ...
    function fetchCategory1() {
        axios.get('http://localhost:8000/GetCategory1')
            .then(response => {
                setProducts1(response.data)
                setMarkers(response.data.filter((item) => item.options === "Casting and machining"))
                response.data.map(item => console.log(item.coordinates))
    ...
    function dataFilter(data) {
        console.log("hello", products1.filter((item) => item.options === data))
    ...
    useEffect(() => {
        ...
        // setMarkers(products1.filter((item)=>item.options==="Casting and machining"))
    }, [products1])
        ...
        <div className="map">
            ...
            {products.length ?
                ...
                {/* <div className='search-containerdetails'> */}
                    {/* <span className='searchicondetail1'> */}
                        {/* <div className="dropdowndetails1"> */}
                        ...
                        {/* </div> */}
                    {/* </span> */}
                    {/* <input type='text'  className='searchdetails' placeholder='Search tool/equipment name' /> */}
                </div>
            {/* </div> */}
                    <ComposableMap>
                        <ZoomableGroup
                        ...
                        >
                            ...
                            {markers[0]?.GetPoints.map(item => {
                                return (
                                    <>
                                        <Marker data-tooltip-id="my-tooltip-1" data-tooltip-html={`status:${item.status}<br></br>ToolID:${item.tool_id}`} coordinates={item.location}>
                                            <g className="tooltip"
                                                fill="none"
                                                stroke="black"
                                                strokeWidth="1"
                                            ...
                                            {/* <text className="tooltiptext" display={false} >Hover me</text> */}
                                            <text textAnchor="middle" x={4} y={7} fill="red" fontSize={"8px"}>
                                                {item.location_name}
                                            </text>
                                            {/* <text textAnchor="left" x={5} y={10} fill="green" fontSize={"8px"}>
          {item.toolCondition}
        </text> */}
                                        </Marker>
                                    </>)
